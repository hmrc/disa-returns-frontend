  ...
  ".keepAlive" - {
    ...
    "when there is a record for this id" - {
      ...
      "must update its lastUpdated to `now` and return true" in {
        ...
        val expectedUpdatedSession = session copy (lastUpdated = instant)

        val updatedSession = find(Filters.equal("_id", session.id)).futureValue.headOption.value
        result mustEqual true
        updatedSession mustEqual expectedUpdatedSession
      }
    }
    ...
    mustPreserveMdc(repository.keepAlive(session.id))
  }
